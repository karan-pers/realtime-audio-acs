...
async def add_vectors(collection: AzureAISearchCollection, vectorizer: VectorStoreRecordUtils):
    ...
    """
    output = [res async for res in await collection.search_client.search(select="hotel_id")]
    print("output", output)
    ids:list[str] = [res.get("hotel_id") for res in output]
    ...
    if hotels is not None and isinstance(hotels, list):
        for hotel in hotels:
            if not hotel.description_vector or not hotel.description_fr_vector:
                hotel = await vectorizer.add_vector_to_records(records= hotel, data_model_type=HotelSampleClass)
