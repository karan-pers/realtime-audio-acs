...
class HelperPlugin:
    ...
    def get_weather(self, location: str) -> str:
        ...
        weather_conditions = ("sunny", "hot", "cloudy",
                              "raining", "freezing", "snowing")
        weather = weather_conditions[randint(
            0, len(weather_conditions) - 1)]  # nosec
...
kernel.add_plugin(plugin=HelperPlugin(), plugin_name="helpers",
                  description="Helper functions for the realtime client.")

kernel.add_plugin(plugin=HotelSearch(), plugin_name="knowledge_base_hotel",
                  description="Functions to query knowledge base for hotels")
            ...
            if data["kind"] == "AudioData":
                ...
                await client.send(
                    event=RealtimeAudioEvent(
                        audio=AudioContent(
                            data=data["audioData"]["data"], data_format="base64", inner_content=data),
                    )
                )
        except Exception:
    ...
    for event in await request.json:
        ...
        match event["type"]:
            case "Microsoft.Communication.CallConnected":
                ...
                app.logger.info("CORRELATION ID:--> %s",
                                event_data["correlationId"])
                app.logger.info("CALL CONNECTION ID:--> %s",
                                event_data["callConnectionId"])
            ...
            case "Microsoft.Communication.MediaStreamingFailed":
                ...
                app.logger.info(
                    f"Message:->{event_data['resultInformation']['message']}")
